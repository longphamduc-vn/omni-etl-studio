...
with tab_converter:
    ...
    render_converter_widget()import streamlit as st
...
# Nút Refresh lại danh sách file JSON trong thư mục workflows/
if st.sidebar.button("🔄 Refresh Workflows"):
    registry._scan_and_load_workflows()

available_workflows = registry.list_workflows()

if not available_workflows:
    st.sidebar.error("No valid workflow JSON files found in 'workflows/' directory.")
    st.stop()
